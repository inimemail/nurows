...
export function normalizeOrchestrationState(parsed = {}) {
  ...
  normalized.probeTargets = normalized.probeTargets.map(({ quorumMode, quorumCount, ...target }) => target);
...
export function retryWaitingIpIncidents(deps, encryptionKey = null) {
  ...
  const recovered = state.incidents.filter((incident) => incident.status === 'waiting_for_ip' && state.probeTargets.find((target) => target.id === incident.targetId)?.health === 'healthy');
  if (recovered.length) {
    deps.updateState((draft) => {
  ...
  const waiting = state.incidents.filter((item) => {
    ...
    const freshRecheck = item.recheckRequestedAt && Date.parse(target?.lastCheckAt || 0) >= Date.parse(item.recheckRequestedAt);
...
function normalizeResource(key, input = {}, existing = null, deps) {
  ...
  if (key === 'probeTargets') return { ...base, name: requiredText(input.name, '目标名称'), address: validateHost(input.address, Boolean(input.allowPrivate)), allowPrivate: Boolean(input.allowPrivate), checkType: input.checkType === 'tcp' ? 'tcp' : 'ping', port: clampNumber(input.port, 1, 65535, 443), interval: clampNumber(input.interval, 5, 3600, 30), timeout: clampNumber(input.timeout, 1, 60, 5), failureThreshold: clampNumber(input.failureThreshold, 1, 20, 3), recoveryThreshold: clampNumber(input.recoveryThreshold, 1, 20, 2), probeIds: cleanIds(input.probeIds, 500), policyId: cleanId(input.policyId), enabled: input.enabled !== false, health: existing?.health || 'unknown', observations: existing?.observations || {}, lastCheckAt: existing?.lastCheckAt || '', checkNowAt: existing?.checkNowAt || '' };
...
export function evaluateTargetHealth(target, probes) {
  ...
  const observations = Object.entries(target.observations || {}).filter(([probeId, observation]) => {
    return assignedProbeIds.includes(probeId) && Date.now() - Date.parse(observation.checkedAt || 0) < Math.max(120000, target.interval * 4000);
  });
  const completeRound = assignedProbeIds.length > 0 && observations.length === assignedProbeIds.length;
  const failed = completeRound && observations.every(([, item]) => !item.ok && item.failures >= target.failureThreshold);
  const healthy = completeRound && observations.some(([, item]) => item.ok && item.successes >= target.recoveryThreshold);
