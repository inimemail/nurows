            ...
            <section className="automation-board">
              ...
              <div className="automation-workspace-grid">
                <div className="automation-left-column">
                  <div className="surface automation-summary-panel"><div className="workspace-head"><div><strong>任务配置</strong><span>只读摘要</span></div></div>{selectedAutomationTask ? <div className="automation-summary-list"><div><span>SSH</span><strong>{selectedAutomationTask.username}@目标:{selectedAutomationTask.port}</strong></div><div><span>步骤</span><strong>{selectedAutomationTask.steps?.length || 0}</strong></div><div><span>并发</span><strong>{selectedAutomationTask.concurrency}</strong></div><div><span>代理</span><strong>{state.proxies.find((item) => item.id === selectedAutomationTask.proxyId)?.name || '直连'}</strong></div><div><span>Telegram</span><strong>{selectedAutomationTask.telegramEnabled ? '允许执行' : '未开放'}</strong></div></div> : <div className="empty-state">请选择左侧任务</div>}</div>
