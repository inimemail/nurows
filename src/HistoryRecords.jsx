...
export const HISTORY_LABELS = {
  ...
  ipUsageRecords: 'IP 使用记录', dnsChanges: 'DNS 变更记录', auditLogs: '审计记录'
};

const STATUS = { running: '执行中', queued: '等待执行', paused: '已暂停', awaiting_input: '等待输入', done: '已完成',
  healthy: '正常', replaced: '已完成补位', degraded: '容量不足', error: '执行异常', waiting_ip: '等待备用 IP' };
...
export default function HistoryRecords({ scope, api, revision, clearing, onClear }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    api(`/api/history/${scope}?page=${page}`, { signal: AbortSignal.any([controller.signal, AbortSignal.timeout(15000)]) })
      .then((data) => { if (!controller.signal.aborted) setResult(data); })
      .catch((err) => { if (!controller.signal.aborted) setError(err.message); })
      .finally(() => { if (!controller.signal.aborted) setLoading(false); });
    return () => controller.abort();
  }, [api, scope, page, refresh, revision]);
  ...
  return <>
    <div className="ops-content-head">
      ...
      <div className="ops-content-actions">
        <button className="ghost danger-text-button" disabled={clearing} onClick={() => onClear(scope)}>{clearing ? '清理中...' : '清理全部'}</button>
    ...
    {error ? <p role="alert" className="auth-error">{error}</p> : loading ? <p className="confirm-copy">正在读取记录...</p> : <div className="ops-list">
      {records.length ? records.map((item) => {
        ...
        const title = scope === 'automationRuns' ? item.taskName : scope === 'dnsGuardRuns' ? item.guardName || item.domain : item.summary || item.action;
        ...
        return <article className="ops-row" key={item.id}><div className="ops-row-main"><div><strong>{title || '历史记录'}</strong><span>{[time ? new Date(time).toLocaleString('zh-CN') : '', detail].filter(Boolean).join(' · ')}</span></div></div>
