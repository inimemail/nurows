...
export default function OrchestrationWorkspace({ tab, state, api, onState, toast, Dialog }) {
  ...
  const [guardRemote, setGuardRemote] = useState({ open: false, guardId: '', name: '', recordType: 'A', values: '', expectedValues: [], busy: false });
  ...
  const openGuardRemote = async (guard) => {
    try {
      ...
      setGuardRemote({ open: true, guardId: guard.id, name: guard.name, recordType: guard.recordType, values: (data.values || []).join('\n'), expectedValues: data.values || [], busy: false });
    } catch (error) { toast(error.message); }
  };
  const refreshGuardRemote = async () => {
    if (!guardRemote.guardId || guardRemote.busy) return;
    setGuardRemote((current) => ({ ...current, busy: true }));
    try {
      ...
      setGuardRemote((current) => ({ ...current, busy: false }));
      toast(error.message);
    }
  };
  const saveGuardRemote = async () => {
    ...
    try {
      ...
      onState(data.state);
      setGuardRemote({ open: false, guardId: '', name: '', recordType: 'A', values: '', expectedValues: [], busy: false });
  ...
  return (
    <section className="ops-workspace">
      ...
      {guardRemote.open ? <Dialog title={`管理远程 IP · ${guardRemote.name}`} className="ops-editor-dialog" wide onClose={() => !guardRemote.busy && setGuardRemote({ open: false, guardId: '', name: '', recordType: 'A', values: '', expectedValues: [], busy: false })} footer={<><button className="ghost" disabled={guardRemote.busy} onClick={refreshGuardRemote}>{guardRemote.busy ? '处理中...' : '重新读取远程'}</button><div className="dialog-actions"><button className="ghost" disabled={guardRemote.busy} onClick={() => setGuardRemote({ open: false, guardId: '', name: '', recordType: 'A', values: '', expectedValues: [], busy: false })}>取消</button><button className="primary" disabled={guardRemote.busy} onClick={saveGuardRemote}>{guardRemote.busy ? '写入中...' : '写入远程'}</button></div></>}><div className="guard-remote-editor"><div className="confirm-copy">当前快照包含 {guardRemote.expectedValues.length} 个 {guardRemote.recordType} 地址。保存前会检查远程记录是否被其他操作修改，并在写入后重新验证。</div><Field label={`${guardRemote.recordType} 地址（每行一个）`} full><textarea rows="14" value={guardRemote.values} onChange={(event) => setGuardRemote((current) => ({ ...current, values: event.target.value }))} placeholder={guardRemote.recordType === 'AAAA' ? '2001:db8::1' : '1.1.1.1'} /></Field></div></Dialog> : null}
...
function renderSection(section, ctx) {
  ...
  if (section === 'guards') return <DataView title="DNS 多 IP 守护" copy="解析 DDNS 主备来源，逐 IP 检查并从备用池自动补位" action="新增守护任务" onAction={() => ctx.openCreate('guard')} empty="还没有 DNS 守护任务">{state.dnsGuards?.map((item) => { const tone = item.status === 'healthy' || item.status === 'replaced' ? 'ok' : ['error', 'waiting_ip'].includes(item.status) ? 'bad' : 'warn'; const busy = Boolean(item.cycle); return <Row key={item.id} title={item.name} subtitle={`${item.recordType} · ${item.domain} · ${item.currentValues?.length || 0}/${item.maxActiveIps || 50} 个活动 IP · ${item.sources?.length || 0} 个来源 · ${item.probeIds?.length || 0} 个探针 · ${formatTime(item.lastCheckAt)}`} status={STATUS[item.status] || item.status} tone={tone} onTripleClick={() => ctx.duplicateRecord('guard', item)} actions={<><button className="ghost" onClick={() => ctx.openGuardView(item)}>查看</button><button className="ghost" onClick={() => ctx.syncGuardRemote(item.id).catch((error) => ctx.toast(error.message))}>读取远程</button><button className="ghost" disabled={busy} onClick={() => ctx.openGuardRemote(item)}>管理 IP</button><button className="ghost" disabled={busy} onClick={() => ctx.checkGuardNow(item.id)}>检查并修复</button><button className="ghost" onClick={() => ctx.openEdit('guard', item)}>编辑规则</button></>} />; })}</DataView>;
  ...
  return <DataView title="解析记录变更" copy="记录每次远端 DNS 写入的变更前后值；故障事件可整组回滚" empty="还没有解析记录变更" emptyCopy="保存解析绑定或执行故障切换后会自动生成记录。">{state.dnsChanges?.map((item) => {
...
function DataView({ title, copy, action, secondaryAction, dangerAction, onAction, onSecondaryAction, onDangerAction, empty, emptyCopy = '使用右上角操作开始配置。', children }) {
