...
export default function OrchestrationWorkspace({ tab, state, search = '', onSearchChange, onSearchScopeChange, api, onState, toast, Dialog, onHistoryCleanup, onOpenHistory, historyRevision, historyClearing }) {
  ...
  const duplicateRecord = async (type, source) => {
    ...
    if (type === 'guard') Object.assign(next, { domain: '', alertBotIds: [], status: 'queued', message: '', currentValues: [], ownedValues: [], sourceOwnedValues: [], sourceState: {}, cycle: null, lastCheckAt: '', nextCheckAt: '', lastError: '', providerRecordIds: [] });
...
function GuardEditor({ value, patch, state, api }) {
  ...
  return <EditorGrid>
    ...
    <div className="guard-notification-hint">默认不选机器人。仅删除不健康 IP 时通知，并显示剩余活动 IP 数量；正常检查不通知。</div>
