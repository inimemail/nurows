...
export default function OrchestrationWorkspace({ tab, state, api, onState, toast, Dialog, onHistoryCleanup, onOpenHistory, historyRevision, historyClearing }) {
...
function renderSection(section, ctx) {
  ...
  if (section === 'guards') return <DataView title="DNS 多 IP 守护" copy="解析 DDNS 主备来源，逐 IP 检查并从备用池自动补位" action="新增守护任务" secondaryAction="检查记录" onSecondaryAction={() => ctx.onOpenHistory('dnsGuardRuns')} onAction={() => ctx.openCreate('guard')} empty="还没有 DNS 守护任务">{state.dnsGuards?.map((item) => { const tone = item.status === 'healthy' || item.status === 'replaced' ? 'ok' : ['error', 'waiting_ip'].includes(item.status) ? 'bad' : 'warn'; const busy = Boolean(item.cycle); const syncing = ctx.guardSyncingIds.includes(item.id); return <Row key={item.id} title={item.name} subtitle={`${item.recordType} · ${item.domain} · ${item.currentValues?.length || 0}/${item.maxActiveIps || 50} 个活动 IP · ${item.sources?.length || 0} 个来源 · ${item.probeIds?.length || 0} 个探针 · ${formatTime(item.lastCheckAt)}`} status={STATUS[item.status] || item.status} tone={tone} onTripleClick={() => ctx.duplicateRecord('guard', item)} actions={<><button className="ghost" onClick={() => ctx.openGuardView(item)}>查看</button><button className="ghost" disabled={syncing} onClick={() => ctx.syncGuardRemote(item.id).catch((error) => ctx.toast(error.message))}>{syncing ? '读取中...' : '读取远程'}</button><button className="ghost" onClick={() => ctx.openGuardRemote(item)}>管理 IP</button><button className="ghost" disabled={busy} onClick={() => ctx.checkGuardNow(item.id)}>检查并修复</button><button className="ghost" onClick={() => ctx.openEdit('guard', item)}>编辑规则</button></>} />; })}</DataView>;
...
function UsageRecordsView({ records, onClear, clearing }) {
