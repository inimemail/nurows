...
function renderSection(section, ctx) {
  ...
  if (section === 'targets') return <DataView title="检查目标" copy="每次 3 轮、每轮 3 次；任一探针任意一次成功即正常" action="新增目标" onAction={() => ctx.openCreate('target')} empty="还没有检查目标">{state.probeTargets?.map((item) => <Row key={item.id} title={item.name} subtitle={targetSubtitle(item)} status={STATUS[item.health] || item.health} tone={item.health === 'healthy' ? 'ok' : item.health === 'down' ? 'bad' : 'warn'} onTripleClick={() => ctx.duplicateRecord('target', item)} actions={<><button className="ghost" onClick={() => ctx.checkTargetNow(item.id)}>立即检查</button><button className="ghost" onClick={() => ctx.openEdit('target', item)}>编辑</button></>} />)}</DataView>;
...
function targetSubtitle(item) { const observations = Object.values(item.observations || {}).sort((a, b) => Date.parse(b.checkedAt || 0) - Date.parse(a.checkedAt || 0)); const latest = observations[0]; const result = !latest ? '尚未检查' : latest.ok ? `第 ${latest.successfulRound || 1} 轮第 ${latest.successfulAttempt || 1} 次成功` : latest.attempts === 9 ? '9 次全部失败' : '等待探针升级'; return `${item.checkType === 'ping' ? 'PING' : `TCP:${item.port}`} · ${item.address || '未填写地址'} · ${item.probeIds?.length || 0} 个探针 · 3轮×3次 · ${result} · ${formatTime(item.lastCheckAt)}`; }
function probeVersionLabel(version) { const parts = String(version || '').split('.').map(Number); const current = parts[0] > 1 || (parts[0] === 1 && parts[1] >= 1); return version ? `${version}${current ? '' : '（需升级）'}` : '未接入'; }
