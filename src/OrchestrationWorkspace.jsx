...
function DnsChangeRow({ item, binding, account }) {
  ...
  return <article className="dns-change-row"><div className="dns-change-title"><span className={`ops-dot ${rolledBack ? 'muted' : 'ok'}`} /><div><strong>{domain}</strong><span>{recordType} · {provider} · {formatTime(item.createdAt)}</span></div></div><div className="dns-change-values"><div><span>变更前</span><code>{displayChangeValues(item.beforeValues)}</code></div><div><span>变更后</span><code>{displayChangeValues(item.afterValues)}</code></div></div><em className={`ops-status ${rolledBack ? 'muted' : 'ok'}`}>{rolledBack ? '已回滚' : '已应用'}</em></article>;
}
