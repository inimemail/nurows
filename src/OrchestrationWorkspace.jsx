...
const STATUS = { online: '在线', offline: '离线', pending: '待接入', revoked: '已吊销', healthy: '正常', down: '故障', observing: '观察中', unknown: '未检测', queued: '等待执行', waiting_for_ip: '等待备用 IP', recovered: '目标已恢复', pending_approval: '待确认', allocating: '分配 IP', automating: '执行任务', dns_updating: '更新 DNS', verifying: '验证中', processing: '处理中', discarded: '检测不可用，已丢弃', consumed: '成功消耗', succeeded: '已完成', failed: '失败保留', rolled_back: '已回滚' };

const EMPTY = {
  probe: { name: '', region: '', carrier: '', maxConcurrency: 100, enabled: true },
  target: { name: '', address: '', allowPrivate: false, checkType: 'ping', port: 443, interval: 30, timeout: 5, failureThreshold: 3, recoveryThreshold: 2, probeIds: [], policyId: '', enabled: true },
  ...
  return (
    <section className="ops-workspace">
      ...
      <div className="surface ops-content">
        {renderSection(active, { state, openCreate, openEdit, duplicateRecord, checkTargetNow, openProbeInstall, rotateProbe, executeIncident, rollbackIncident, openAssetImport: () => setAssetImport((current) => ({ ...current, open: true })), api, onState, toast })}
      </div>
...
function renderSection(section, ctx) {
  ...
  if (section === 'targets') return <DataView title="检查目标" copy="每轮所有负责探针检查，任一探针成功即正常" action="新增目标" onAction={() => ctx.openCreate('target')} empty="还没有检查目标">{state.probeTargets?.map((item) => <Row key={item.id} title={item.name} subtitle={`${item.checkType === 'ping' ? 'PING' : `TCP:${item.port}`} · ${item.address || '未填写地址'} · ${item.probeIds?.length || 0} 个探针`} status={STATUS[item.health] || item.health} tone={item.health === 'healthy' ? 'ok' : item.health === 'down' ? 'bad' : 'warn'} onTripleClick={() => ctx.duplicateRecord('target', item)} actions={<><button className="ghost" onClick={() => ctx.checkTargetNow(item.id)}>立即检查</button><button className="ghost" onClick={() => ctx.openEdit('target', item)}>编辑</button></>} />)}</DataView>;
